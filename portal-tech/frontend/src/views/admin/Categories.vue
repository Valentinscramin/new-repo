  ...
  <div class="min-h-screen flex items-start justify-center">
    ...
    <div class="flex justify-between items-center mb-6">
      ...
      <div class="flex gap-3">
        ...
        <button @click="showCreateForm = true" class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded transition">+ New Category</button>
      </div>
    </div>

    <!-- Create/Edit Form -->
    <div v-if="showCreateForm || editingItem" class="mb-6 p-4 bg-gray-900 rounded-lg border border-gray-800">
      <h3 class="text-lg font-semibold mb-4">{{ editingItem ? 'Edit' : 'Create' }} Category</h3>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <input v-model="form.name" placeholder="Name" class="px-4 py-2 bg-gray-800 rounded border border-gray-700 focus:outline-none focus:border-blue-500" />
        <input v-model="form.slug" placeholder="Slug" class="px-4 py-2 bg-gray-800 rounded border border-gray-700 focus:outline-none focus:border-blue-500" />
      </div>
      <div class="flex gap-3">
    ...
    <LoadingSpinner v-if="loading" message="Carregando categorias..." />
    <div v-else-if="items.length === 0" class="text-gray-400">No categories yet.</div>
...
async function save() {
  try {
    if (editingItem.value) {
      await api.put(`/admin/categories/${editingItem.value.id}`, form.value);
    } else {
    ...
    console.error(e);
    const msg = e?.response?.data?.error || 'Error saving category';
    alert(msg);
  }
}
...
async function deleteItem(id) {
  ...
  try {
    ...
    console.error(e);
    const msg = e?.response?.data?.error || 'Error deleting category';
    alert(msg);
  }
}
